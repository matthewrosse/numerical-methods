...
struct Matrix *gauss_elimination_method(struct Matrix *matrix,
  ...
  struct Matrix **upper_triangular_result =
      matrix_get_upper_triangular_with_constant_terms(matrix, constant_terms);
...
matrix_get_upper_triangular_with_multipliers(struct Matrix *matrix) {
  ...
  struct Matrix *l_matrix = matrix_init(matrix->rows, matrix->cols);
  struct Matrix *u_matrix = matrix_clone(matrix);
  ...
  for (size_t i = 0; i < n; i++) {
    for (size_t j = 0; j < n; j++) {
      ...
      if (j > i) {
        multiplier = u_matrix->data[j][i] / u_matrix->data[i][i];

        l_matrix->data[j][i] = multiplier;

        for (size_t k = i; k < n + 1; k++) {
          if (k != n) {
            u_matrix->data[j][k] =
                u_matrix->data[j][k] - multiplier * u_matrix->data[i][k];
          }
        }
      }
    }
  ...
  result_array[0] = l_matrix;
  result_array[1] = u_matrix;
